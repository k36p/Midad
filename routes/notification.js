...
router.post('/notifications/add', AccessPanel, upload.single('mediaFile'), async (req, res) => {
   try {
      ...
      if (err.code === 11000) {
         res.status(500).json({
            message: 'هنالك رسالة حالية من نفس البريد  الألكتروني، إنتظر حتى يتم الرد عليها قبل الإرسال مرة أخرى'
         });
      } else {
         res.status(500).json({
            message: 'فشل في إرسال الرسالة، حاول لاحقا'
         });
      }
...
router.get('/notifications/all', AccessPanel, async (req, res) => {
   try {
      let data = await Notify.find({}).sort({
         createdAt: -1
      }).populate('specialization').lean();
      res.json(data);
   } catch (err) {
      res.status(500).json({
         message: 'فشل في جلب الإشعارات، حاول لاحقا'
      });
   }
...
// @ to do:
/*
- convert plain messages to json messages from msg.json
*/