...
router.get('/data/user/:id',AccessPanel, async (req, res) => {
    ...
    if(!UserId){
return res.status(500).json({message: messages.ar.USERNAME_REQUIERD})
    }

    try {
        const user = await User.findById(UserId); // get users data
        if (!user) {
            return res.status(404).json({ message: messages.ar.UNDEFIEND_USERNAME });
        }
        res.json(user);
    } catch (error) {
        console.error(error);
...
router.post('/admins/update/:action/:id',OwnerOremission, async (req, res) => {
    try {
        ...
        await user.save();
        res.json({ message: messages.ar.PREMISSION_REMOVED_SUCCESSFULLY });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: messages.ar.SERVER_ERROR });
    }
