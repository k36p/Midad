   ...
   try {
      ...
      if (!name || name.trim() === '') {
         return res.status(400).json({
            error: true,
            message: messages.ar.SPECIALIZATION_NAME_REQUIERD
         });
      }
      if (!collegeId || collegeId.trim() === '') {
         return res.status(400).json({
            error: true,
            message: messages.ar.SPECIALIZATION_COLLEGE_REQUIERD
         });
      }
      ...
      if (!existingCollege) {
         return res.status(404).json({
            error: true,
            message: messages.ar.UNDEFIEND_COLLEGE
         });
      }
      ...
      if (existingSpecialization) {
         return res.status(409).json({
            error: true,
            message: messages.ar.SPECIALIZATION_ALREDY_CREATED
         });
      }
      ...
      res.status(201).json({
         message: messages.ar.SPECIALIZATION_CREATED_SUCCESFULLY,
      ...
      res.status(500).json({
         error: true,
         message: messages.ar.SPECIALIZATION_FAILD_CREATE,
         details: err.message
      });
   }
});
...
router.get('/specializations/all', async (req, res) => {
   try {
      ...
      res.status(500).json({
         error: true,
         message: messages.ar.SPECIALIZATION_COLLECT_FROM_SERVER_ERROR
      });
   }
});
...
router.get('/specializations/:college', async (req, res) => {
   try {
      ...
      if (specializations.length === 0) {
         return res.status(404).json({
            message: messages.ar.NO_SPECILIZATIONS_FOR_COLLEGE
         });
      }
      ...
      res.status(500).json({
         error: true,
         message: messages.ar.SPECIALIZATIONS_FETCH_ERROR
      });
   }
});
...
router.get('/data/specialization/:id', AccessPanel, async (req, res) => {
   try {
      ...
      res.status(500).json({
         error: true,
         message: messages.ar.SPECIALIZATION_DATA_FETCH_ERROR
      });
   }
})
...
router.post('/specialization/update/:id', async (req, res) => {
   ...
   if (!name && !college) {
      return res.status(400).json({
         message: messages.ar.NAME_COLLEGE_REQUIERD
      });
   }
   if (name.trim() === '') {
      return res.status(400).json({
   ...
   if (!specializationData) {
      return res.status(404).json({
         message: messages.ar.UNDEFIEND_SPECILIZATION
      });
   }
   try {
      ...
      res.status(200).json({
         message: messages.ar.SPECIALIZATION_UPDATED_SUCCESSFULLY
      });
      ...
      res.status(500).json({
         error: true,
         message: messages.ar.SPECIALIZATION_UPDATE_FAIL
      });
   }
})
